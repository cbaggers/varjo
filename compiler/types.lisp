...
(defmethod type->type-spec ((type v-fake-struct)) 
  (v-fake-type-name type))
...
(defun type-spec->type (spec &key place (env *global-env*))
  (cond ((null spec) (error 'unknown-type-spec :type-spec spec))
        ((and (not (eq env *global-env*)) (get-fake-type spec env))
         (get-fake-type spec env))
...
(defmethod v-typep ((a v-type) (b v-type) &optional (env *global-env*))
  ...
  (or (typep a 'v-stemcell)
      (typep a (type->type-spec b))))
(defmethod v-typep ((a v-type) b &optional (env *global-env*))
  (cond ((typep a 'v-stemcell) t)
        ((symbolp b) (or (get-fake-type b env) (typep a b)))
        ((and (listp b) (numberp (second b))) 
         (or (get-fake-type (first b) env) (typep a (first b))))))
